...
asio::awaitable<void> EthereumRpcApi::handle_eth_get_transaction_by_hash(const nlohmann::json& request, nlohmann::json& reply) {
    ...
    try {
        ethdb::TransactionDatabase tx_database{*tx};
        const auto optional_block_and_transaction = co_await core::read_transaction_by_hash(*context_.block_cache, tx_database, transaction_hash);
        if (!optional_block_and_transaction) {
            // TODO(sixtysixter)
            ...
            reply = make_json_content(request["id"], optional_block_and_transaction->transaction);
        }
    } catch (const std::invalid_argument& iv) {
        SILKRPC_DEBUG << "invalid_argument: " << iv.what() << " processing request: " << request.dump() << "\n";
        reply = make_json_content(request["id"], {});
