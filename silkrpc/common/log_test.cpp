...
TEST_CASE("parse log level", "[silkrpc][common][log]") {
    ...
    for (auto i{0}; i < input_texts.size(); i++) {
        LogLevel level;
        std::string error;
        const auto success{silkrpc::AbslParseFlag(input_texts[i], &level, &error)};
        CHECK(success == true);
        CHECK(error.empty());
...
TEST_CASE("parse invalid log level", "[silkrpc][common][log]") {
    LogLevel level;
    std::string error;
    const auto success{silkrpc::AbslParseFlag("abc", &level, &error)};
    CHECK(success == false);
    CHECK(!error.empty());
}
...
TEST_CASE("unparse log level", "[silkrpc][common][log]") {
    ...
    for (auto i{0}; i < input_levels.size(); i++) {
        const auto text{silkrpc::AbslUnparseFlag(input_levels[i])};
...
TEST_CASE("LOG macro does nothing for lower verbosity", "[silkrpc][common][log]") {
    ...
    SILKRPC_LOG_STREAMS(ss1, silkrpc::null_stream());
...
TEST_CASE("LOG macro does output for higher or equal verbosity", "[silkrpc][common][log]") {
    ...
    SILKRPC_LOG_STREAMS(ss2, silkrpc::null_stream());
...
TEST_CASE("SILKRPC_TRACE macro uses level Trace", "[silkrpc][common][log]") {
    std::stringstream ss;
    SILKRPC_LOG_STREAMS(ss, silkrpc::null_stream());
