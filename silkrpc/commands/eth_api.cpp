...
asio::awaitable<void> EthereumRpcApi::handle_eth_get_block_by_hash(const nlohmann::json& request, nlohmann::json& reply) {
    ...
    try {
        ...
        const auto block_with_hash = co_await core::read_block_by_hash(context_, tx_database, block_hash);
...
asio::awaitable<void> EthereumRpcApi::handle_eth_get_block_by_number(const nlohmann::json& request, nlohmann::json& reply) {
    ...
    try {
        ...
        const auto block_with_hash = co_await core::read_block_by_number(context_, tx_database, block_number);
...
asio::awaitable<void> EthereumRpcApi::handle_eth_estimate_gas(const nlohmann::json& request, nlohmann::json& reply) {
    ...
    try {
        ...
        const auto latest_block_with_hash = co_await core::read_block_by_number(context_, tx_database, latest_block_number);
    ...
    try {
        ...
        for (auto block_to_match : block_numbers) {
            ...
            if (filtered_block_logs.size() > 0) {
                const auto block_with_hash = co_await core::read_block_by_number(context_, tx_database, block_to_match);
