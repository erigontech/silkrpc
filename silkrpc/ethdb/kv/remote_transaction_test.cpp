...
TEST_CASE_METHOD(RemoteTransactionTest, "RemoteTransaction::open", "[silkrpc][ethdb][kv][remote_transaction]") {
    ...
    SECTION("failure in request") {
        ...
        CHECK_THROWS_MATCHES(spawn_and_wait(remote_tx_.open()), boost::asio::error_code, test::exception_has_cancelled_grpc_status_code());
    }
...
TEST_CASE_METHOD(RemoteTransactionTest, "RemoteTransaction::close", "[silkrpc][ethdb][kv][remote_transaction]") {
    SECTION("throw w/o open") {
        ...
        CHECK_THROWS_AS(spawn_and_wait(remote_tx_.close()), boost::asio::error_code);
    }
    ...
    SECTION("failure in write") {
        ...
        CHECK_THROWS_MATCHES(spawn_and_wait(remote_tx_.close()), boost::asio::error_code, test::exception_has_cancelled_grpc_status_code());
    }
    SECTION("failure in finish") {
        ...
        // Execute the test: closing the transaction should raise an exception w/ expected gRPC status code
        CHECK_THROWS_MATCHES(spawn_and_wait(remote_tx_.close()), boost::asio::error_code, test::exception_has_unknown_grpc_status_code());
    }
}
...
TEST_CASE_METHOD(RemoteTransactionTest, "RemoteTransaction::cursor", "[silkrpc][ethdb][kv][remote_transaction]") {
    SECTION("throw w/o open") {
        ...
        CHECK_THROWS_AS(spawn_and_wait(remote_tx_.cursor("table1")), boost::asio::error_code);
    }
    SECTION("success") {
        // Set the call expectations:
        // 1. remote::KV::StubInterface::PrepareAsyncTxRaw call succeeds
    ...
    SECTION("failure in read") {
        ...
        CHECK_THROWS_MATCHES(spawn_and_wait(remote_tx_.cursor("table1")), boost::asio::error_code,
            test::exception_has_cancelled_grpc_status_code());

        // Execute the test postconditions:
        // close the transaction raises an exception w/ expected gRPC status code
        CHECK_THROWS_MATCHES(spawn_and_wait(remote_tx_.close()), boost::asio::error_code,
            test::exception_has_cancelled_grpc_status_code());
    }
    SECTION("failure in write") {
        // Set the call expectations:
...
TEST_CASE_METHOD(RemoteTransactionTest, "RemoteTransaction::cursor_dup_sort", "[silkrpc][ethdb][kv][remote_transaction]") {
    SECTION("throw w/o open") {
        ...
        CHECK_THROWS_AS(spawn_and_wait(remote_tx_.cursor_dup_sort("table1")), boost::asio::error_code);
    ...
    SECTION("failure in read") {
        ...
        // Execute the test: opening a cursor should raise an exception w/ expected gRPC status code
        CHECK_THROWS_MATCHES(spawn_and_wait(remote_tx_.cursor_dup_sort("table1")), boost::asio::error_code,
            test::exception_has_cancelled_grpc_status_code());
