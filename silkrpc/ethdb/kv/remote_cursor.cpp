...
asio::awaitable<void> RemoteCursor2::open_cursor(const std::string& table_name) {
    const auto start_time = clock_time::now();
    if (cursor_id_ == 0) {
        SILKRPC_DEBUG << "RemoteCursor::open_cursor opening new cursor for table: " << table_name << "\n";
...
asio::awaitable<KeyValue> RemoteCursor2::seek(silkworm::ByteView key) {
    const auto start_time = clock_time::now();
    SILKRPC_DEBUG << "RemoteCursor::seek cursor: " << cursor_id_ << " key: " << key << "\n";
...
asio::awaitable<KeyValue> RemoteCursor2::seek_exact(silkworm::ByteView key) {
    const auto start_time = clock_time::now();
    SILKRPC_DEBUG << "RemoteCursor::seek_exact cursor: " << cursor_id_ << " key: " << key << "\n";
    auto seek_message = remote::Cursor{};
...
asio::awaitable<KeyValue> RemoteCursor2::next() {
    const auto start_time = clock_time::now();
...
asio::awaitable<silkworm::Bytes> RemoteCursor2::seek_both(silkworm::ByteView key, silkworm::ByteView value) {
    const auto start_time = clock_time::now();
    SILKRPC_DEBUG << "RemoteCursor::seek_both cursor: " << cursor_id_ << " key: " << key << " subkey: " << value << "\n";
    auto seek_message = remote::Cursor{};
...
asio::awaitable<KeyValue> RemoteCursor2::seek_both_exact(silkworm::ByteView key, silkworm::ByteView value) {
    const auto start_time = clock_time::now();
    SILKRPC_DEBUG << "RemoteCursor::seek_both_exact cursor: " << cursor_id_ << " key: " << key << " subkey: " << value << "\n";
    auto seek_message = remote::Cursor{};
...
asio::awaitable<void> RemoteCursor2::close_cursor() {
    const auto start_time = clock_time::now();
    const auto cursor_id = cursor_id_;
    if (cursor_id_ != 0) {
        SILKRPC_DEBUG << "RemoteCursor::close_cursor closing cursor: " << cursor_id_ << "\n";
