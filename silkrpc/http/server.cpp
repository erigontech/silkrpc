...
Server::Server(const std::string& end_point, const std::string& api_spec, ContextPool& context_pool, asio::thread_pool& workers, std::string jwt_secret)
...
asio::awaitable<void> Server::run() {
    ...
    try {
        while (acceptor_.is_open()) {
            ...
            auto new_connection = std::make_shared<Connection>(context, workers_, handler_table_, jwt_secret_);
