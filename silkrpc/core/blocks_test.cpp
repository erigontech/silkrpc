...
static std::string block_hash = "0x439816753229fc0736bf86a5048de4bc9fcdede8c91dadf88c828c76b2281dff";
...
TEST_CASE("get_block_number", "[silkrpc][core][blocks]") {
    ...
    SECTION("kFinalizedBlockId") {
        ...
        const auto view_hash = silkworm::bytes_of_string(block_hash);
        EXPECT_CALL(db_reader, get(db::table::kLastForkchoice, _)).WillOnce(InvokeWithoutArgs(
            [&]() -> asio::awaitable<KeyValue> { 
                co_return KeyValue{silkworm::Bytes{}, view_hash}; 
            }
        ));
