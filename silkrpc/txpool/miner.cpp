...
Miner::Miner(boost::asio::io_context& context, std::shared_ptr<grpc::Channel> channel, grpc::CompletionQueue* queue)
: Miner(context.get_executor(), ::txpool::Mining::NewStub(channel), queue) {}

Miner::Miner(boost::asio::io_context::executor_type executor, std::unique_ptr<::txpool::Mining::StubInterface> stub, grpc::CompletionQueue* queue)
...
boost::asio::awaitable<WorkResult> Miner::get_work() {
    ...
    const auto reply = co_await get_work_awaitable.async_call(::txpool::GetWorkRequest{}, boost::asio::use_awaitable);
...
boost::asio::awaitable<bool> Miner::submit_work(const silkworm::Bytes& block_nonce, const evmc::bytes32& pow_hash, const evmc::bytes32& digest) {
    ...
    const auto reply = co_await submit_work_awaitable.async_call(submit_work_request, boost::asio::use_awaitable);
...
boost::asio::awaitable<bool> Miner::submit_hash_rate(const intx::uint256& rate, const evmc::bytes32& id) {
    ...
    const auto reply = co_await submit_hashrate_awaitable.async_call(submit_hashrate_request, boost::asio::use_awaitable);
    const auto ok = reply.ok();
...
boost::asio::awaitable<uint64_t> Miner::get_hash_rate() {
    ...
    const auto reply = co_await hashrate_awaitable.async_call(::txpool::HashRateRequest{}, boost::asio::use_awaitable);
...
boost::asio::awaitable<MiningResult> Miner::get_mining() {
    ...
    const auto reply = co_await mining_awaitable.async_call(::txpool::MiningRequest{}, boost::asio::use_awaitable);
