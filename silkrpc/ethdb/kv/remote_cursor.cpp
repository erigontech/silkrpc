...
boost::asio::awaitable<void> RemoteCursor::open_cursor(const std::string& table_name) {
    ...
    if (cursor_id_ == 0) {
        ...
        cursor_id_ = co_await kv_awaitable_.async_open_cursor(table_name, boost::asio::use_awaitable);
...
boost::asio::awaitable<KeyValue> RemoteCursor::seek(silkworm::ByteView key) {
    ...
    auto seek_pair = co_await kv_awaitable_.async_seek(cursor_id_, key, boost::asio::use_awaitable);
...
boost::asio::awaitable<KeyValue> RemoteCursor::seek_exact(silkworm::ByteView key) {
    ...
    auto seek_pair = co_await kv_awaitable_.async_seek_exact(cursor_id_, key, boost::asio::use_awaitable);
    const auto k = silkworm::bytes_of_string(seek_pair.k());
    const auto v = silkworm::bytes_of_string(seek_pair.v());
...
boost::asio::awaitable<KeyValue> RemoteCursor::next() {
    const auto start_time = clock_time::now();
    auto next_pair = co_await kv_awaitable_.async_next(cursor_id_, boost::asio::use_awaitable);
...
boost::asio::awaitable<silkworm::Bytes> RemoteCursor::seek_both(silkworm::ByteView key, silkworm::ByteView value) {
    ...
    auto seek_pair = co_await kv_awaitable_.async_seek_both(cursor_id_, key, value, boost::asio::use_awaitable);
    const auto k = silkworm::bytes_of_string(seek_pair.k());
    const auto v = silkworm::bytes_of_string(seek_pair.v());
...
boost::asio::awaitable<KeyValue> RemoteCursor::seek_both_exact(silkworm::ByteView key, silkworm::ByteView value) {
    ...
    auto seek_pair = co_await kv_awaitable_.async_seek_both_exact(cursor_id_, key, value, boost::asio::use_awaitable);
    const auto k = silkworm::bytes_of_string(seek_pair.k());
    const auto v = silkworm::bytes_of_string(seek_pair.v());
...
boost::asio::awaitable<void> RemoteCursor::close_cursor() {
    ...
    if (cursor_id_ != 0) {
        ...
        co_await kv_awaitable_.async_close_cursor(cursor_id_, boost::asio::use_awaitable); // Can we shoot and forget? Or avoid and delegate to CORE service cleanup?
